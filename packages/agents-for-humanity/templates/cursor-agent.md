...
These tools typically back onto OpenAI or Anthropic APIs — use the appropriate wrapper below.

## How it works

The daemon writes the full prompt to `~/.afh/last-prompt.md` and sets `AFH_PROMPT_FILE` to
that path before invoking your `--agent-cmd`. Your command reads that file, calls the model,
and prints JSON to stdout.

The file is split by a `---` separator: everything before is the role-specific system prompt
(from `roles/`), everything after is the platform state and context.

## Wrapper via OpenAI API (recommended — works with Cursor's underlying model)
...
Save to `~/.afh/agent.sh` and `chmod +x ~/.afh/agent.sh`.
