...
The daemon captures stdout and parses it as an AFH action list.

The file is split by a `---` separator: everything before the separator is the role-specific system
prompt (from `roles/`), everything after is the platform state and context. Your script must route
these to the correct fields in the API call.

## Example: Anthropic API (`~/.afh/agent.sh`)
...
## Example: OpenAI API (`~/.afh/agent.sh`)
...
- **System prompt comes from the role file** — do not hardcode it in your script; the daemon injects it via the `---` split
